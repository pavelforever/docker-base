# docker-base
> Если этот docker установлен в lxc контейнере, сначала добавь внешние порты (останови контейнер сначала ```lxc stop КОНТЕЙНЕР```). ЧТОБ БЫСТРЕЕ БЫЛО, МОЖЕШЬ В SUBLIME TEXT ВСТАВИТЬ И ТАМ ЗАМЕНИТЬ `КОНТЕЙНЕР` на свой. Ну или плейбук написать в ansible.
